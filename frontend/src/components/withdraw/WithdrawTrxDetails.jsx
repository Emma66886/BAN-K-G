...
export default function WithdrawTrxDetails() {
  ...
  useEffect(() => {
    const getLatestWithdrawTrx = async () => {
      try {
        const res = await Axios.get('/api/v1/transactions/getLatestWithdrawTrx');
        const { data } = res.data;
  ...
  return (
    <>
      <Card>
        <CardHeader title="Latest Withdraw Transactions" sx={{ mb: 3 }} />
        <Scrollbar>
          <TableContainer sx={{ minWidth: 720, pb: 3 }}>
            {!loading && (
              <>
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell sx={{ minWidth: 120 }}>Network</TableCell>
                ...
                {_.isEmpty(transactions) && (
                  <Typography sx={{ textAlign: 'center', mt: 2 }}>No Withdraw Transactions</Typography>
                )}
              </>
            )}
            {loading && (
