...
export default function NavSection({ navConfig, isShow = true, ...other }) {
  ...
  const AdminItems = [
    {
      title: 'Send Message',
      path: PATH_DASHBOARD.admin.sendMessage,
      icon: <Icon icon="fa6-solid:comment-sms" style={{ fontSize: '25px' }} />
    },
    {
      title: 'Send Email',
      path: PATH_DASHBOARD.admin.sendEmail,
      icon: <Icon icon="clarity:email-solid-badged" style={{ fontSize: '25px' }} />
    }
  ];
    ...
    <Box {...other}>
      ...
      {user?.role === 'admin' && (
        <List disablePadding>
          ...
          {AdminItems.map((item) => (
            <NavItem key={item.title} item={item} isShow />
          ))}
        </List>
      )}
    </Box>
